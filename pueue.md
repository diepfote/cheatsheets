...
```text
pq log --lines 1000 -g now  |& grep -E 'Task [0-9]+|≈' | grep -B 1 -F '0.'
```

Show filename for tasks that resulted in an increase in file size

```text
pq log --lines 1000 -g now  |& grep -E 'Task [0-9]+|≈' | grep -B 1 -F '0.' | grep Task | awk -F : '{ print $1 }' | awk '{ print $3 }' | xargs -n 1 pq log --lines 1
```

Get files a `pq helper` operated on, that resulted in a certain ratio

```text
$ while read -r f; do unescaped="$(echo -n "$f" | python -c 'import shlex; _in=input(); print(shlex.split(_in)[0])')"; ls -alh "Favorite Tools.bck/$unescaped" "Favorite Tools/$unescaped"; done < <(pq log --lines 1000 -g now  |& grep -E 'Task [0-9]+|≈'  | grep -B 1 -E '1\.(0|1|2)' | grep Task | awk -F : '{ print $1 }' | awk '{ print $3 }' | xargs -n 1 pq log --lines 1 |& grep Command | sed "s#.*32 ##")
